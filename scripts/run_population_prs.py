#!/usr/bin/env python3
"""
Population-specific PRS calculation script.

Example usage:
python run_population_prs.py --config config/myconfig.ini --gwas data/gwas/cad_gwas.csv 
                             --bfile data/genotypes/1000g --pop_file data/population/pop_info.csv 
                             --populations EAS,SAS --use_clumping
                             --gwas_build GRCh37 --genotype_build GRCh38
                             --out cad_pop_prs
"""
...
from src.population_handler import PopulationHandler
from src.utils import Utils
from src.genome_build import GenomeBuildHandler

def parse_args():
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Calculate population-specific PRS.")
    ...
    parser.add_argument("--pop_file", required=True, help="File mapping individuals to populations")
    parser.add_argument("--populations", help="Comma-separated list of populations to analyze")
...
def main():
    """Main function to run population-specific PRS calculation."""
    # Parse arguments
    args = parse_args()
    ...
    # Set up logging
    logger = Utils.setup_logging(os.path.join(config.get_path("output_dir"), "population_prs.log"))
    logger.info("Starting population-specific PRS calculation")
    ...
    try:
        ...
        pop_handler = PopulationHandler(config.get_path("temp_dir"), chain_dir)
        ...
        logger.info(f"Loading population data from {args.pop_file}")
        pop_data = pop_handler.load_population_data(args.pop_file)
        pop_summary = pop_handler.summarize_population_data(pop_data)
        
        # Determine which populations to analyze
        if args.populations:
            target_pops = [p.strip() for p in args.populations.split(",")]
        else:
            target_pops = config.get_populations()
        
        logger.info(f"Target populations: {', '.join(target_pops)}")
        
        # Calculate PRS for each population
        population_profiles = {}
        
        for pop in target_pops:
            logger.info(f"Processing population: {pop}")
            
            # Create keep file for this population
            keep_file = pop_handler.create_population_keep_file(pop_data, pop)
            logger.info(f"Created keep file for {pop}")
            
            # Calculate PRS
            output_name = f"{args.out}_{pop}"
            
            if args.use_clumping:
                logger.info(f"Calculating clumped PRS for {pop}")
                profile_file = calculator.clumped_prs(
                    gwas_file=args.gwas,
                    bfile=args.bfile,
                    ...
                    keep_samples=keep_file,
                    source_build=gwas_build,
                    target_build=genotype_build
                )
            else:
                logger.info(f"Calculating basic PRS for {pop}")
            ...
            stats_file = os.path.join(config.get_path("output_dir"), f"{output_name}_stats.txt")
            pop_stats = pop_handler.calculate_population_statistics(profile_file, stats_file)
            
            logger.info(f"Completed PRS calculation for {pop}")
            logger.info(f"Mean: {pop_stats['mean']:.4f}, SD: {pop_stats['std']:.4f}, n={pop_stats['n_samples']}")
            
            population_profiles[pop] = profile_file
        
        logger.info("Population-specific PRS calculation complete")
        ...
        logger.error(f"Error in population PRS calculation: {str(e)}", exc_info=True)
        return 1
